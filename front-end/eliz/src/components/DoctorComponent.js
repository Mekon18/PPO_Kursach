...
class Doctors extends Component {
    ...
    render() {
        ...
        return (
            <CardDeck>
                {people.map(person => <Col md={4}><CardComponent image={person.Id == 2 ? miha : sveta} title={<Nav.Link href={"/profile/" + person.Id}>{person.Name}</Nav.Link>} text={person.Speciality} /></Col>)}
            </CardDeck>
        );
    }
}
